...
let fullName = [];
runners.forEach(function(business) {
  fullName.push(business.first_name + " " + business.last_name);
});
...
ticketPriceTotal = runners.map(runner => runner.donation).reduce(reducer);
// console.log('$'+ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1: Company and Email for Marketing campaign at next year's race:
let marketing = [];
marketing = runners.map(runner => runner.company_name + " " + runner.email);
// console.log(marketing);
// Problem 2: Runner id numbers must be listed with t-shirt size so we can print number on shirt.
let jerseyNumber = [];
jerseyNumber = runners.map(
  runner => runner.id + " will need to be on a size " + runner.shirt_size
);
// console.log(jerseyNumber);
// Problem 3 Donation tier for banquet recognition.
let banquet = [];
banquet = runners
  ...
  .reverse();
// console.log(JSON.stringify(banquet));
let biggest = [];
biggest = runners
  .filter(runner => runner.donation > "200")
  .map(runner => " " + runner.company_name + " with $" + runner.donation);
console.log("Our GOLD tier donors for the race were:");
console.log("" + biggest);

let big = [];
big = runners
  .filter(runner => runner.donation < "200" && runner.donation > "100")
  .map(runner => " " + runner.company_name + " with $" + runner.donation);
console.log("Our SILVER tier donors for the race were:");
console.log("" + big);
let mention = [];
mention = runners
  .filter(runner => runner.donation < "100"&&runner.donation>"50")
  .map(runner => " " + runner.company_name + " with $" + runner.donation);
console.log("Our BRONZE tier donors for the race were:");
console.log(""+mention);
let poor = [];
poor = runners
  .filter(runner => runner.donation<"50")
  .map(runner => " " + runner.company_name + " with $" + runner.donation);
console.log("The rest of you need to donate more next year, and that includes:");
console.log(""+poor);
console.log("That is chump change guys, we can do better than that!!!");
